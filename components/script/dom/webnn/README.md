...
- **When adding multiple `MLGraphBuilder` methods in one patch:** every trait method should include a method-level WebNN anchor doc-comment and explicit `Step N:` comments, even when the body delegates to a shared helper. For delegating methods, include the method-level steps and add a `TODO:` immediately under any step whose algorithm data (for example per-op attributes like axis/scales/epsilon) is not yet persisted.
...
- **Private helper naming:** do not use generic suffixes like `_impl` for helper methods that implement spec algorithms. Use spec-derived names (for example `create_reduction_operation`, `create_a_pooling_operation`) and add a method-level WebNN anchor doc-comment for the algorithm or section being implemented.

- Do not route methods through a shared helper unless the method steps explicitly call that named helper algorithm in the spec. For WebNN element-wise operators, methods in `#api-mlgraphbuilder-binary`, `#api-mlgraphbuilder-logical`, and `#api-mlgraphbuilder-unary` delegate to their corresponding helper algorithms, while methods such as `sigmoid`, `softplus`, `softsign`, and `reverse` follow their own method steps directly.
